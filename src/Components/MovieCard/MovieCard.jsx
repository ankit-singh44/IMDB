...
const MovieCard = (props) => {
  ...
  const addToWatchList=()=>{
    ...
    if(watchList===null){
      updatedWatchList=[props.movieObj];
    }
    else{
      updatedWatchList=[...watchList,props.movieObj];
    }
  ...
  return (
    <div className='h-96 w-48 bg-cover bg-center flex flex-col justify-between items-end m-4 rounded-xl shadow-lg transform overflow-hidden hover:scale-110 hover:cursor-pointer duration-300' style={{backgroundImage: `url(https://image.tmdb.org/t/p/original/${props.movieObj.poster_path})`}}>

      {
        ...
        <div className='mt-4'>
        <span onClick={addToWatchList} className='m-4 mt-8 p-3 rounded-full' style={{fontSize: '22px',backgroundColor: 'rgba(31, 41, 55, 0.6)'}}>&#128525;</span>
      </div> :
      <div className='mt-4'>
        <span onClick={removeFromWatchList} className='m-4 mt-8 p-3 rounded-full' style={{fontSize: '22px',backgroundColor: 'rgba(31, 41, 55, 0.6)'}}>&#10060;</span>
      </div>
      }
